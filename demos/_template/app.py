        ...
        for h in target_hours:
            tmp = next(
                (
                    float(i["fcstValue"])
                    for i in items
                    if i["category"] == "TMP" and i["fcstTime"].startswith(h)
                ),
                None,
            )
            pop = next(
                (
                    float(i["fcstValue"])
                    for i in items
                    if i["category"] == "POP" and i["fcstTime"].startswith(h)
                ),
                None,
            )
            TMPs[h] = tmp
    ...
    if "location_name" in st.session_state and "week_dates" in st.session_state:
        ...
        with st.spinner(f"{location_name} 평일 점심 예보 확인 중..."):
            for d in week_dates:
                ...
                pm10, pm10_grade, _pm10_time = get_air_quality(location_name)
        ...
        if today_tmp_dict:
            st.caption(
                f"오늘(성남시 분당구) 12/13시 기온: {today_tmp_dict.get('12')}°C / {today_tmp_dict.get('13')}°C"
            )
    else:
